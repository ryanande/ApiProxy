...
namespace EdFiValidation.ApiProxy.Helpers.CustomBindings
{
    public class CatchAllRouteBinding : HttpParameterBinding
    {
        private readonly string _parameterName;
        private readonly char _delimiter;

        public CatchAllRouteBinding(HttpParameterDescriptor descriptor, char delimiter)
            : base(descriptor)
        {
        ...
        public override Task ExecuteBindingAsync(System.Web.Http.Metadata.ModelMetadataProvider metadataProvider, HttpActionContext actionContext, CancellationToken cancellationToken)
        {
            ...
            {
                string[] catchAllValues = routeValues[_parameterName].ToString().Split(_delimiter);
